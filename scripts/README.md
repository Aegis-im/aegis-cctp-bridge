...
### DepositProxyV2 (Devnet)
```bash
# Admin wallet (upgrade authority / vault admin)
export ANCHOR_PROVIDER_URL=https://api.devnet.solana.com
export ANCHOR_WALLET=./devnetUser.json
...
# Init vault (uses REMOTE_EVM_DOMAIN + REMOTE_EVM_ADDRESS + optional DESTINATION_CALLER from .env)
npm run proxy-v2-admin init-vault -- --vaultId $DEPOSIT_PROXY_V2_VAULT_ID

# Create manager keypair (transfer_manager)
solana-keygen new -o manager.json --no-bip39-passphrase
solana airdrop -u devnet 2 manager.json
MANAGER_PUBKEY=$(solana-keygen pubkey manager.json)

# Set manager
npm run proxy-v2-admin set-manager -- --vaultId $DEPOSIT_PROXY_V2_VAULT_ID --newManager $MANAGER_PUBKEY

# Fund vault ATA with 0.002 USDCSOL (USDC devnet mint by default)
# Get vault ATA without npm banners
VAULT_ATA=$(./node_modules/.bin/tsx proxyV2Admin.ts vault-ata --vaultId $DEPOSIT_PROXY_V2_VAULT_ID)
spl-token transfer -u devnet --owner $ANCHOR_WALLET 4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU 0.002 $VAULT_ATA
